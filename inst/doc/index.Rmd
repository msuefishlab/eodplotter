...
First try using the `tdmsplot` command, it plots the raw signal as a graph, use to diagnose signal problems
```
tdmsplot -f file.tdms
```


Then use `peak_finder` to find all EODs and outputs a new file, e.g. file.peaks.csv, which is named based on the input file (if you analyzed fish.tdms, it would output fish.peaks.csv)
